...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
    ...
    refuelStore.register({
      let refuels = self.refuelStore.all().sorted(by: self.createdDateSorter)
      WatchSessionManager.sharedManager.updateApplicationContext(Dashboard().toDictionary(refuels) as [String : AnyObject])
    })
  ...
  func application(_ application: UIApplication,
    ...
    if (shortcutItem.type == "com.motingo.Autometry.RecordRefuel") {
      window!
        ...
        .performSegue(withIdentifier: "RecordRefuel", sender: self)
    }
  }
